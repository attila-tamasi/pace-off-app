...
- `PaceOffTests` — XCTest suite (`PushTargetEngineTests`, `TrainingPlanGeneratorTests`, `GoalPredictionServiceTests`, `HealthDataCacheTests`); unit tests are cheap and expected
...
- Pure algorithms live in `PaceOffShared` — `Sendable`, no I/O, unit-tested: `PushTargetEngine` (daily target, PRD §7), `TrainingPlanGenerator` + `VDOTCalculator` (both in `TrainingPlanGenerator.swift`; Daniels' formula closed-form, no lookup tables), `GoalPredictionService`
